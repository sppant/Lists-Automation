...
class New_offer extends CI_Controller {
										...
										foreach( $period as $index => $date ) {
											...
											if ($spot_way != 'kil') {
												if ($repeat_every==$day) {
												 ...
												 $sql = "INSERT INTO offer(customer_name, spot_time, spot_date,repeat_every, spot_show, spot_type, spot_way, priority, spot_category) VALUES ('$customer_name', '$spot_time','$newDate','$repeat_every', '$spot_show', '$spot_type', '$spot_way', '$priority', '$spot_category')";
												 $mysqli->query($sql);
												}
											}
	...
	{
		$offer_spot_name = $_POST['offer_spot_name'];

		$this->load->model('offers');
		$data['offer_spot_name'] = $this->offers->find_spot_name($offer_spot_name);

		if ($data['offer_spot_name'] == []){
			$datas= false;
		}else {
			$datas=true;
		}
